...
输入题目内容、Pilot Top 25候选及对应老师原释义。DS逐设问输出所有被直接考查的Label；允许不同Label存在语义重叠，但不输出仅作为上位概念、材料背景、底层常识或弱相关联想的Label。每个命中Label必须附题目、答案或解析中的原文证据及对应设问，同时记录原候选排名，用于验证20与25的取舍。
