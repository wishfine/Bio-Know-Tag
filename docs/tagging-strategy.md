...
当前实现边界：题目清洗、打标单元、父题聚合计划、精确去重、无旧标签Pilot抽样、字符n-gram BM25、可配置Transformers Dense召回、BM25/Dense分歧报告、配额融合及候选DS精判已经实现；“全458名称+`@`路径”的DS粗召回仅保留为历史实验。taxonomy软路由和最终父题结果聚合尚未运行或实现。本文后续章节同时定义这些待实现阶段的数据契约和验收标准。
...
Pilot分歧审核显示BM25在教材术语、遗传计算、生态工程和细胞自噬等题目上明显优于小型Dense，但Dense能补充语义改写和超长多主题题的部分内容。因此第一版不做等权RRF，而采用BM25主导的配额融合：
...
Pilot先保留25个候选，并记录DS最终选中Label的候选排名。若人工确认的正确Label几乎不落在21～25名，生产版本降为20个以减少Prompt成本；若尾部5项持续补回正确Label，则保留25个。困难题可扩展到30～50个。
...
DS粗召回目录使用 `B001` 至 `B458` 的短代码，响应后再映射到真实 `label_id`，避免模型抄错19位ID。轻微格式错误按可审计原则归一化：同一道题内的重复短代码保持首次出现顺序去重，超过Top-K时保留前K项；未知短代码、缺少题目或题目乱序仍判为失败。原始响应和归一化计数必须写入evidence。

## 10. DS裁决策略

### 10.1 独立题

输入题目内容、Pilot Top 25候选及对应老师原释义。候选在发送给DS前按题目ID确定性打乱以控制位置偏置，但结果仍记录原召回排名。DS逐设问输出所有被直接考查的Label；允许两个Label在具有独立考查依据时语义重叠，但不输出仅作为上位概念、材料背景、底层常识或弱相关联想的Label。错误选项只有在判断其正误确实需要该知识时才计入。每个命中Label必须附题目、答案或解析中的原文证据及对应设问，同时区分 `need_expand_recall`（候选覆盖不足）与 `context_insufficient`（缺图或缺父题材料导致信息不足），并记录原候选排名，用于验证20与25的取舍。

生成训练数据时采用紧凑v5输出：只要求Label短代码和最短题内证据；候选缺失时额外输出简短 `missing_knowledge`。拒绝项、必要性解释和全局理由不作为训练目标，减少解码时延并降低JSON截断概率。
...
- 输出所有被实际设问直接考查的知识点；允许语义重叠，不输出仅仅相关的知识点。
...
经Pilot确定上限后的候选集进入一次DS联合裁决，是默认主路由。
