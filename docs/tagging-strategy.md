...
当前实现边界：题目清洗、打标单元、父题聚合计划、精确去重、无旧标签Pilot抽样、字符n-gram BM25、可配置Transformers Dense召回，以及BM25/Dense分歧报告已经实现；“全458名称+`@`路径”的DS粗召回仅保留为历史实验。RRF核心融合函数已实现，但taxonomy软路由、Top-20 DS精判和最终父题结果聚合尚未运行或实现。本文后续章节同时定义这些待实现阶段的数据契约和验收标准。
